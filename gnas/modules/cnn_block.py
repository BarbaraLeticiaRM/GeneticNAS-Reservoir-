...
class CnnSearchModule(nn.Module):
    def __init__(self, n_channels, ss, individual_index=0, se_block=True):
        ...
        if self.ss.single_block:
            self.weights = [Parameter(torch.Tensor(n_channels, n_channels, 1)) for _ in range(len(ss.ocl))]
        else:
            self.weights = [Parameter(torch.Tensor(n_channels, n_channels, 1)) for _ in
    ...
    def forward(self, inputs_tensor, bypass_input):
        ...
        result = F.conv1d(self.relu(net), w, self.bias)
        net = self.bn(result)
        var = self.se_block(net)
        return var + inputs_tensor
