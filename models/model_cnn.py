...
class Net(nn.Module):
    def __init__(self, n_blocks, n_channels, n_classes, dropout, ss, aux=False):
        ...
        if(args.dataset_name == 'MIT'):
            self.conv1 = nn.Conv1d(1, n_channels, 3, stride=1, padding=1, bias=False)
        elif(args.dataset_name == 'EEG'):
            self.conv1 = nn.Conv1d(64, n_channels, 3, stride=1, padding=1, bias=False)
        ...
        self.conv2 = nn.Conv1d(n_channels, 2 * n_channels, 1, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm1d(2 * n_channels)

        self.conv2_prev = nn.Conv1d(n_channels, 2 * n_channels, 1, stride=1, padding=1, bias=False)
        self.bn2_prev = nn.BatchNorm1d(2 * n_channels)
        # self
        ...
        self.conv3 = nn.Conv1d(2 * n_channels, 4 * n_channels, 1, stride=1, padding=1, bias=False)
        self.bn3 = nn.BatchNorm1d(4 * n_channels)

        self.conv3_prev = nn.Conv1d(2 * n_channels, 4 * n_channels, 1, stride=1, padding=1, bias=False)
        self.bn3_prev = nn.BatchNorm1d(4 * n_channels)
